...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$user) {
        ...
        if ($member && password_verify($p, $member['password'])) {
            ...
            $uid = $member['user_id'];
            $mem_id = $member['id'];
            $mem_name = $member['nama'];
            mysqli_query($conn, "INSERT INTO activity_logs (user_id, family_id, activity_type, description) VALUES ($uid, $mem_id, 'login', 'Member $mem_name masuk ke sistem')");
    ...
    if ($user && password_verify($p, $user['password'])) {
        ...
        if (isset($_SESSION['user_id'])) {
            ...
            mysqli_query($conn, "INSERT INTO activity_logs (user_id, activity_type, description) VALUES ($uid, 'login', '$u_name masuk ke sistem')");
        }
        if ($user['role'] == 'admin') {
