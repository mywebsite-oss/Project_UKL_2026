...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        if (password_verify($p, $row['password'])) {
            ...
            $prefix = ($row['role'] == 'user') ? "User " : "";
            mysqli_query($conn, "INSERT INTO activity_logs (user_id, activity_type, description) VALUES ({$row['id']}, 'login', '{$prefix}{$row['username']} berhasil masuk.')");
    ...
    if (mysqli_num_rows($q2) > 0) {
        ...
        if (password_verify($p, $row2['password'])) {
            ...
            mysqli_query($conn, "INSERT INTO activity_logs (user_id, activity_type, description) VALUES ({$row2['admin_id']}, 'login', 'Member {$row2['username']} berhasil masuk.')");
